...
def new_H(u, ammo, attacker,cont, iter, HMCR=0.95):
    temp = random.random()
    PAR_min, PAR_max = 0.08, 0.12
    band_min, band_max = 0.8, 1
    PAR = PAR_max - (PAR_max - PAR_min) * cont / iter
    band = band_max * math.exp(math.log(band_min / band_max) * cont / iter)
    if temp < HMCR:
        new_indi = []
        #print(u[0])
        for i in range(len(u[0])):
            ...
            if random.random() < PAR:
                x = int(x + 2 * band * random.randint(0, 1) - band)
            new_indi.append(max(x, 0))
    else:
        new_indi = HM_init(ammo, attacker)
    return new_indi
def IHS(ammo, c, attacker, p_mtoa, iteration = 1, p_yu = 0.95):
    ...
    plt.figure(1)
    plt.plot(plot_x, plot_y, color='yellow', linestyle='-', label='基于IHS的方法')
    plt.xlabel('迭代次数')
    plt.ylabel('平均毁伤概率')
    plt.legend(loc='best')
    plt.figure(2)
    plt.plot(plot_x, plot_z, color='yellow', linestyle='-', label='基于IHS的方法')
...
p_yu = 0.95
HS_indi, HS_f, HS_cost, HS_time = IHS(ammo, c, attacker, p_mtoa, iteration=500)

...
def new_H(u, ammo, attacker, HMCR=0.95, PAR=0.1, band=1):
    ...
    return new_indi
def GHS(ammo, c, attacker, p_mtoa, iteration = 500, p_yu = 0.95):
    ...
    plt.figure(1)
    plt.plot(plot_x, plot_y, color='red', linestyle='-', label='基于GHS的方法')
    plt.xlabel('迭代次数')
    plt.ylabel('平均毁伤概率')
    plt.legend(loc='best')
    plt.figure(2)
    plt.plot(plot_x, plot_z, color='red', linestyle='-', label='基于GHS的方法')

...
def sort_u(u, p_mtoa, ammo, c, len_u=5, p_yu=0.95):
    ...
    res = sorted_u[:len_u:]
    return res
def new_H(u, ammo, attacker, HMCR=0.95, PAR=0.1, band=1):
    temp = random.random()
    if temp < HMCR:
        new_indi = []
        #print(u[0])
        for i in range(len(u[0])):
            ...
            if random.random() < PAR:
                x = x + 2 * band * random.randint(0, 1) - band
            new_indi.append(max(x, 0))
    else:
        new_indi = HM_init(ammo, attacker)
    return new_indi
def HS(ammo, c, attacker, p_mtoa, iteration = 1, p_yu = 0.95):
    ...
    while(i < iteration):
        i += 1
        new_indi = new_H(u, ammo, attacker)
        u.append(new_indi)
    ...
    plt.figure(1)
    plt.plot(plot_x, plot_y, color='green', linestyle='--', label='基于HS的方法')
    plt.xlabel('迭代次数')
    plt.ylabel('平均毁伤概率')
    plt.figure(2)
    plt.plot(plot_x, plot_z, color='green', linestyle='--', label='基于HS的方法')

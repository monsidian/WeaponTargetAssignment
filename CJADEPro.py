...
class CjadePro:
    def __init__(self, iterNum, popNum, instance, pMutation=0.2, uF=0.5, uCR=0.1, c=0.5):
        ...
        # self.bestInd = max(self.uOri, key=lambda x: x.fitness)
    ...
    def crossOver(self, uMutant):
        ...
        for indCross in uCross:
            indCross.CR = np.random.normal(self.uCR, 0.1)
            if np.random.random() < indCross.CR:
    ...
    def update(self):
        ...
        for i in range(self.iterNum):
            ...
            self.bestInd = u[0]
            # self.bestInd = max(u, key=lambda x: x.fitness)
